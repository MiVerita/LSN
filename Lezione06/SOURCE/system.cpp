...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    //spin dei primi vicini
    double s_left = _particle(this->pbc(i-1)).getspin();  
    double s_right = _particle(this->pbc(i+1)).getspin();
    //probabilità spin +1
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
      } else if( property == "SPECIFIC_HEAT" ){ //I posti riservati in _measurement sono 2: uno per la misura del quadrato dell'energia e quello successivo per il calcolo di cv a fine blocco
        _nprop+=2;
        _measure_cv = true;
...
void System :: measure(){ // Measure properties
  ...
  if(_measure_magnet){
    double sum_spin = 0;
    for(int i = 0; i < _npart; i++){
      sum_spin += _particle(i).getspin();
    }
    _measurement(_index_magnet) = sum_spin / _npart;
  }
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if(_measure_cv){
    _measurement(_index_cv) = pow(tenergy_temp * _npart,2);
    _measurement(_index_cv+1) = 0;
  }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if(_measure_chi){
    double sum_spin = 0;
    ...
    _measurement(_index_magnet) = _beta * sum_spin * sum_spin / _npart;
  }
...
void System :: averages(int blk){
  ...
  if(_measure_cv){
    _average(_index_cv+1) = _beta * _beta * (_average(_index_cv) - pow(_average(_index_tenergy)*_npart,2))/_npart;
  }
  ...
  // TOTAL ENERGY //////////////////////////////////////////////////////////////
  if (_measure_tenergy and blk == _nblocks){
    if(_sim_type == 3){ //caso gibbs
      sum_average = _global_av(_index_tenergy);
      sum_ave2 = _global_av2(_index_tenergy);
      coutf.open("../OUTPUT/energy_vs_temperature_gibbs.dat", ios::app);
      coutf << _temp << " " << sum_average/double(blk) << " " << this->error(sum_average, sum_ave2, blk) <<endl;
      coutf.close();
    }else if(_sim_type == 2){ //caso metropolis
      sum_average = _global_av(_index_tenergy);
      sum_ave2 = _global_av2(_index_tenergy);
      coutf.open("../OUTPUT/energy_vs_temperature_metro.dat", ios::app);
      coutf << _temp << " " << sum_average/double(blk) << " " << this->error(sum_average, sum_ave2, blk) <<endl;
      coutf.close();
    }
  }
  ...
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  if(_measure_magnet and blk == _nblocks){
    if(_sim_type == 3){
      sum_average = _global_av(_index_chi);
      sum_ave2 = _global_av2(_index_chi);
      coutf.open("../OUTPUT/magnetization_vs_temperature_gibbs.dat", ios::app);
      coutf << _temp << " " << sum_average/(double(blk)) << " " << this->error(sum_average, sum_ave2, blk) <<endl;
      coutf.close();
    }else if(_sim_type == 2){
      sum_average = _global_av(_index_chi);
      sum_ave2 = _global_av2(_index_chi);
      coutf.open("../OUTPUT/magnetization_vs_temperature_metro.dat", ios::app);
      coutf << _temp << " " << sum_average/(double(blk)) << " " << this->error(sum_average, sum_ave2, blk) <<endl;
      coutf.close();
    }
  }
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if(_measure_cv and blk == _nblocks){
    if(_sim_type == 3){ //caso gibbs
      sum_average = _global_av(_index_cv+1);
      sum_ave2 = _global_av2(_index_cv+1);
      coutf.open("../OUTPUT/specific_heat_vs_temperature_gibbs.dat", ios::app);
      coutf << _temp << " " << sum_average/(double(blk)) << " " << this->error(sum_average, sum_ave2, blk) <<endl;
      coutf.close();
      ...
      sum_average = _global_av(_index_cv+1);
      sum_ave2 = _global_av2(_index_cv+1);
      coutf.open("../OUTPUT/specific_heat_vs_temperature_metro.dat", ios::app);
      coutf << _temp << " " << sum_average/(double(blk)) << " " << this->error(sum_average, sum_ave2, blk) <<endl;
      coutf.close();
    }
  }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if(_measure_chi and blk == _nblocks){
    if(_sim_type == 3){
      sum_average = _global_av(_index_chi);
      sum_ave2 = _global_av2(_index_chi);
      coutf.open("../OUTPUT/susceptibility_vs_temperature_gibbs.dat", ios::app);
      coutf << _temp << " " << sum_average/(double(blk)) << " " << this->error(sum_average, sum_ave2, blk) <<endl;
      coutf.close();
    }else if(_sim_type == 2){
      sum_average = _global_av(_index_chi);
      sum_ave2 = _global_av2(_index_chi);
      coutf.open("../OUTPUT/susceptibility_vs_temperature_metro.dat", ios::app);
      coutf << _temp << " " << sum_average/(double(blk)) << " " << this->error(sum_average, sum_ave2, blk) <<endl;
      coutf.close();
    }
  }
