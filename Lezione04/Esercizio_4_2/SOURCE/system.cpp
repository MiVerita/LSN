...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    for (int i=0; i<_npart; i++){
      ...
      sumv(0) += vx(i); //drift velocities of the gas
      sumv(1) += vy(i);
      sumv(2) += vz(i);
    }
    */
...
void System :: read_configuration(){
  ...
  if(cinf.is_open()){
    ...
    for(int i=0; i<_npart; i++){
      ...
      _particle(i).setposition(0, this->pbc(_side(0)*x*0.5, 0)); //reticolo riscalato a mezza scatola
...
void System :: print_instant_velocity(){
  ofstream coutf;
  coutf.open("../OUTPUT/pofv_last", ios::app);
  for(int i = 0; i < _npart; i++){
