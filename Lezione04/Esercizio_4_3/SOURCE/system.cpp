...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  _rnd.SetRandom(seed,p1,p2);//incremento indice del numero dell proprietà da misurare
...
void System :: initialize_velocities(){
  ...
  //in questo caso il flag restart è uguale ad 1
  if(_restart){
    ifstream cinf;
    cinf.open("../INPUT/CONFIG/config.xyz"); //in questo caso conf-1 è la configurazione ottenuta al penultimo passo dell'esercizio 4.2
    if(cinf.is_open()){
      string comment;
      string particle;
      int ncoord;
     ...
     /* //Calcolo delle velocità invertite rispetto all'ultimo passo 
      for (int i = 0; i < _npart; i++) {
        vec v(_ndim);
        for (int d = 0; d < _ndim; d++) {
            double x_new = _particle(i).getposition(d, true);   // posizione attuale
            double x_old = _particle(i).getposition(d, false);  // posizione al passo precedente
            v(d) = (x_new - x_old) / (2.0 * _delta);            // velocità stimata
            v(d) *= -1.0;                                       // inversione del tempo
            _particle(i).setvelocity(d, v(d));
        }
        // Ricostruzione di xold per il primo passo all'indietro con Verlet
        vec x_new(_ndim);
        for (int d = 0; d < _ndim; ++d) {
            double x_d = _particle(i).getposition(d, true) - v(d) * _delta;
            x_new(d) = this->pbc(x_d, d); // applica condizioni periodiche
            _particle(i).setpositold(d, x_new(d));
        }
      }*/
    ...
    //ESERCIZIO 4.3 -- In questo caso ho lasciato l'inizializzazione delle velocità come nell'esercizio 4.2
    double v_T = sqrt(3.0 *_temp); //Calcolo la velocità associata all'energia termica unidimensionale
    for(unsigned int i = 0; i < _npart; i++){
      int ax = i % 3; // Assegna la dimensione del vettore ciclicamente
      int cycle = i / 3; // Indice del "ciclo" per alternare il segno
      double sign = (cycle % 2 == 0) ? 1.0 : -1.0;
      vx(i) = (ax == 0) ? sign*v_T : 0.0;
      vy(i) = (ax == 1) ? sign*v_T : 0.0;
      vz(i) = (ax == 2) ? sign*v_T : 0.0;
      sumv(0) += vx(i);
      sumv(1) += vy(i);
      sumv(2) += vz(i);
    }
...
void System :: read_configuration(){
  ifstream cinf;
  cinf.open("../INPUT/CONFIG/conf-1.xyz"); //in questo caso config.xyz è la configurazione ottenuta all'ultimo passo dell'esercizio 4.2
  if(cinf.is_open()){
    string comment;
    string particle;
    double x, y, z;
...
void System :: measure(){ // Measure properties
  ...
  if(_measure_pofv){
    count_particles = 0;
    for(int i = 0; i < _npart; i++){ 
      if(_particle(i).getvelocity_module() < 4.0*_temp){ //4T è la velocità quadra massima in unità ridotte
        count_particles ++;
    ...
    if(count_particles>0){
      for(int i=0;i<_n_bins_v;i++){
        _measurement(_index_pofv + i) /= (double)count_particles; //normalizzazione
...
void System :: averages(int blk){ 
  ...
  // POFV //////////////////////////////////////////////////////////////////////
  if(_measure_pofv && blk == 10){ // Print only the last equilibrated block
...
void System :: print_instant_velocity(){
  ofstream coutf;
  coutf.open("../OUTPUT/pofv_fifth.dat", ios::app);
  for(int i = 0; i < _npart; i++){
    coutf << _particle(i).getvelocity_module() << endl;
  }
  coutf.close();
}
