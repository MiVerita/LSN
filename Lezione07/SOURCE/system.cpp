...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ifstream cinf;
    cinf.open("../INPUT/CONFIG/conf-1_NVE.xyz");
    if(cinf.is_open()){
      string comment;
      string particle;
      int ncoord;
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _vtail = (8*M_PI*_rho)/(9*pow(_r_cut,9))-(8*M_PI*_rho)/(3*pow(_r_cut,3)); //potential tail correction
        ...
        index_property++;
        _ptail = 32*M_PI*_rho*(1/(9*pow(_r_cut,9))-1/(6*pow(_r_cut,3))); //pressure tail correction
...
void System :: read_configuration(){
  ifstream cinf;
  cinf.open("../INPUT/CONFIG/config_NVE.xyz");
  if(cinf.is_open()){
    string comment;
    string particle;
    double x, y, z;
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      ...
      // Solo se dr < r_cut, accumulo contributo potenziale e viriale
        if(dr < _r_cut){
          if(_measure_penergy)  penergy_temp += 1.0/pow(dr,12) - 1.0/pow(dr,6);
...
void System :: averages(int blk){
  ...
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
    coutf.open("../OUTPUT/potential_energy_NVE_ex_7_2.dat",ios::app);
  ...
  if (_measure_pressure){
    coutf.open("../OUTPUT/pressure_NVE_ex_7_4.dat",ios::app);
  ...
  // GOFR //////////////////////////////////////////////////////////////////////
  if(_measure_gofr){
    string gr_blk = "../OUTPUT/gofr_block_" + to_string(blk) + "_NVE.dat";
    coutf.open(gr_blk);
    coutf << "#DISTANCE(r)  GOFR_BLOCK" <<endl;
    //stampo la media di blocco per ogni bin
    for(int bin = 0; bin < _n_bins; bin++){
      double r = (bin + 0.5)*_bin_size;
      double bin_occurence = _block_av(_index_gofr + bin)/double(_nsteps); //media di blocco della misura dell'occurence bin
      double norm = (_npart * (_npart-1)) * 4.0 * M_PI * r * r *_bin_size/_volume; //normalizzazione della distribuzione radiale
      coutf << setw(12) << r << setw(12) << (bin_occurence/norm) <<endl;
    }
    coutf.close();

    coutf.open("../OUTPUT/gofr_NVE.dat");
    //stampo il miglior valore di gr
    for(int bin = 0; bin < _n_bins; bin++){
      double sum_average = _global_av(_index_gofr + bin);
      double sum_ave2 = _global_av2(_index_gofr + bin);
      double error = this->error(sum_average, sum_ave2, _nblocks);
      double r = (bin + 0.5) * _bin_size;
      double norm = (_npart * (_npart-1)) * 4.0 * M_PI * r * r *_bin_size/_volume;
      double gofr = sum_average / (_nblocks * norm);
      double gofr_err = error / norm;
      coutf << setw(12) << r << setw(12) << gofr << setw(12) << gofr_err << endl;
    }
    coutf.close(); 
  }
